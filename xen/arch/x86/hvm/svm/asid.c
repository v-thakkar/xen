...
void svm_asid_init(const struct cpuinfo_x86 *c)
{
   ...
   // data->last_vmcbs = (struct vmcb_struct **)xzalloc_array(void *, nasids);
    /*if ( !svm->last_vmcbs)
        return -ENOMEM;*/
}
...
//            cpu_has_svm_flushbyasid ? TLB_CTRL_FLUSH_ASID : TLB_CTRL_NO_FLUSH;
    ...
    if ( previous_vmcb == vmcb && cpu == svm->last_cpu ) {
        printk (XENLOG_INFO "Yes, it entered here");
        ...
        printk (XENLOG_INFO "It left from here");
        return; 
    }
    ...
    /*if ( vmcb_get_asid(vmcb) > data->hvm->max_asid )
    {
        vmcb->tlb_control =
            cpu_has_svm_flushbyasid ? TLB_CTRL_FLUSH_ASID : TLB_CTRL_NO_FLUSH;
    }*/

    /*if (vmcb->tlb_control != need_flush)
